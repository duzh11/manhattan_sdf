...
def make_dataset(cfg, is_train=True):
    if is_train:
        ...
        module = cfg.train_dataset_module
        path = cfg.train_dataset_path
        args.semantic_class = cfg.model.semantic.semantic_class
    else:
...
def make_data_loader(cfg, is_train=True, is_distributed=False, max_iter=-1):
    ...
    dataset = make_dataset(cfg, is_train) #内含__getitem__，会进行rays的随机抽样
    ...
    sampler = make_data_sampler(dataset, shuffle, is_distributed) #相当于shuffle?
    batch_sampler = make_batch_data_sampler(cfg, sampler, batch_size,
                                            drop_last, max_iter, is_train) #相当于batch
